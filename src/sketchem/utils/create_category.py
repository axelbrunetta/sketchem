...
def get_molecules_for_category_pubchem(api_key, user_prompt):
    ...
    try:
        ...
        if not molecule_names_from_ai:
            return (
                f"Error: AI returned category '{category_header}' but no molecule names.",
                [],
            )

        logger.info(
            f"Created category: {category_header}, Elements: {molecule_names_from_ai}"
        )
            ...
            try:
                ...
                if compounds:
                    ...
                    if compound.canonical_smiles:
                        ...
                        logger.info(
                            f"Found SMILES for '{name}': {compound.canonical_smiles}"
                        )
                    else:
                        logger.info(
                            f"Warning: No canonical SMILES found for '{name}' in PubChem."
                        )
                        # just continue
                        continue
                else:
        ...
        logger.info(
            f"Updated category: {st.session_state.selected_molecule_category}, isDefault: {st.session_state.category_is_default}"
        )

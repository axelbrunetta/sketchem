...
def get_eraser_icon():
    ...
    with open(os.path.join(assets_dir, 'eraser.svg'), 'r') as f:
        return f.read()
...
def render_singleplayer_setup():
    ...
    with col1:
        ...
        # Create button with eraser icon
        button_label = "Switch to Eraser" if st.session_state.drawing_mode == "freedraw" else "Switch to Pen"
        eraser_icon = get_eraser_icon()

...
def process_gemini_category_response(response_text):
    ...
    try:
        ...
        if ":" not in response_text:
            raise ValueError(
                "Invalid response format: No molecule definitions found (missing ':' separator)"
            )
        ...
        logger.info(
            f"Added new category: {category_name} with {len(molecules_dict)} molecules"
        )
        return True
...
def generate_new_category(api_key, user_prompt):
    ...
    if api_key == "TEST_MODE_ENABLED" and (
        user_prompt.lower() == "test"
        or any(
...
"""
            ...
            "drug" in user_prompt.lower()
            or "pharmaceutical" in user_prompt.lower()
            or "medicine" in user_prompt.lower()
        ):
            test_response = """
...
"""
            ...
            "aroma" in user_prompt.lower()
            or "fragrance" in user_prompt.lower()
            or "smell" in user_prompt.lower()
        ):
            test_response = """
...
def render_singleplayer_setup():
    ...
    if st.session_state.toast_queue is not None:
        ...
        if (
            isinstance(toast_data, dict)
            and "message" in toast_data
            and "icon" in toast_data
        ):
    ...
    try:
        ...
        logger.error(f"Could not find CSS file at {style_path}")
        # No fallback styling - the app will use default Streamlit styling

    # page title
    st.markdown(
        "<h2 style='margin-bottom: 20px;'>Single Player Setup</h2>",
    ...
    with col1:
        ...
        if st.session_state.selected_molecule_category in all_categories:
            initial_index = (
                all_categories.index(st.session_state.selected_molecule_category) + 1
            )
        ...
        def openModal():
            ...
            user_input = st.text_input(
                "Category input",  # Add a proper label here
            ...
            if st.button("Submit"):
                try:
                    ...
                    returned_var = get_molecules_for_category_pubchem(
                        api_key=api_key, user_prompt=user_input
                    )
                    ...
                    returned_var = f"Unexpected error: {str(e)}"
        ...
        if st.button(
            "Create a molecule category using AI",
            key="create_category_button",
            help="This is an experimental feature, some things may not work as intended.",
            type="primary",
    ...
    if selected_category and selected_category != "Choose Category":
        ...
        if selected_category in MOLECULE_CATEGORIES:
            ...
            and selected_category in st.session_state.additionalCategories
        ):

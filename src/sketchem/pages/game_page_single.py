...
def handle_submission(canvas_result):
    ...
    if img_bytes is None or Image.open(io.BytesIO(img_bytes)).getcolors() == [
        (400 * 600, (0, 0, 0))
    ]:
    ...
    if game and "category" in game:
        ...
        if category in MOLECULE_CATEGORIES:
            target_smiles = MOLECULE_CATEGORIES[category].get(
                st.session_state.current_molecule
            )
        elif (
            hasattr(st.session_state, "additionalCategories")
            and category in st.session_state.additionalCategories
        ):
            target_smiles = st.session_state.additionalCategories[category].get(
                st.session_state.current_molecule
            )

        if target_smiles:
            # Validate the drawing against the target SMILES
            validation_result = validate_drawing_with_ai(img_bytes, target_smiles)
            ...
            st.error(
                f"Could not find SMILES for molecule {st.session_state.current_molecule}"
            )
            return
    else:
        st.error("Invalid game state")
        return

    if correct:
        st.session_state.points += 1
        st.session_state.toast_queue = {
            "message": f"Correct! You drew {st.session_state.current_molecule} correctly.",
            "icon": "✅",
        }
...
def select_next_molecule():
    ...
    if game and "category" in game:
        ...
        if category in MOLECULE_CATEGORIES:
            ...
            molecules = list(st.session_state.additionalCategories[category].keys())
        else:
        ...
        if molecules:
            ...
            return
        st.error(
            "No molecules found in category. Please go back and choose a valid category."
        )
    else:
...
def render_game_page():
    ...
    try:
        ...
        st.error("Could not find CSS file. Please ensure the styling file exists.")
        return
    ...
    if game and "category" in game:
        ...
        # Initialize first molecule - check both default and custom categories
        if category in MOLECULE_CATEGORIES:
            molecules = list(MOLECULE_CATEGORIES[category].keys())
        elif (
            hasattr(st.session_state, "additionalCategories")
    ...
    with col2:
        if "category" in st.session_state:
            # Get molecules from either default or custom categories
            if st.session_state.category in MOLECULE_CATEGORIES:
            ...
            st.markdown(
                f"**Progress:** {st.session_state.molecule_index}/{total_molecules}"
            )
    ...
    with cols[1]:
        is_white_selected = (
            st.session_state.last_pen_color == "White"
            and st.session_state.drawing_mode != "erase"
        )
    ...
    with cols[2]:
        is_red_selected = (
            st.session_state.last_pen_color == "Red"
            and st.session_state.drawing_mode != "erase"
        )
        red_key = "Red_selected" if is_red_selected else "Red_color"
        st.button(
            "", key=red_key, on_click=select_color, args=("Red",), help="Select Red"
        )

    # Blue button
    with cols[3]:
        is_blue_selected = (
            st.session_state.last_pen_color == "Blue"
            and st.session_state.drawing_mode != "erase"
        )
        blue_key = "Blue_selected" if is_blue_selected else "Blue_color"
        st.button(
            "", key=blue_key, on_click=select_color, args=("Blue",), help="Select Blue"
        )

    # Green button
    with cols[4]:
        is_green_selected = (
            st.session_state.last_pen_color == "Green"
            and st.session_state.drawing_mode != "erase"
        )
    ...
    with cols[5]:
        is_yellow_selected = (
            st.session_state.last_pen_color == "Yellow"
            and st.session_state.drawing_mode != "erase"
        )
    ...
    with cols[6]:
        is_purple_selected = (
            st.session_state.last_pen_color == "Purple"
            and st.session_state.drawing_mode != "erase"
        )
    ...
    with cols[7]:
        ...
        st.button(
            "", on_click=toggle_drawing_mode, key=eraser_key, help=eraser_help_message
        )
    ...
    if st.session_state.game_over:
        if "category" in st.session_state:
            ...
            st.markdown(
                f"## Game Over! Your final score: **{st.session_state.points}/{len(molecules)}**"
            )
